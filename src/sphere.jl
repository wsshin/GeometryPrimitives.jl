...
Sphere(c::SVector{N}, d::Real, data::D=nothing) where {N,D} = Sphere{N,D}(c, 0.5d, data)
Sphere(c::AbstractVector, d::Real, data=nothing) = (N = length(c); Sphere(SVector{N}(c), d, data))
